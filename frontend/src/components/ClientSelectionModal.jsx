...
const ClientSelectionModal = ({
  ...
  useEffect(() => {
    if (isOpen && masterClientId) {
      fetchClients();
    } else if (isOpen && !masterClientId) {
      console.warn("ClientSelectionModal aberto sem masterClientId!");
      setClients([]);
      setIsLoading(false);
      toast.error("ID da empresa cliente não encontrado para filtrar usuários.");
    }
  }, [isOpen, masterClientId]);

  const fetchClients = async () => {
    setIsLoading(true);
    try {
      const url = masterClientId
        ? `${import.meta.env.VITE_BACKEND_URL}/clients?masterClientId=${masterClientId}`
      ...
      if (response.ok) {
        ...
        setClients(
          data.filter(
            (client) => client.isActive && client.MasterClientId === masterClientId
          )
        );
      } else {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden">
        <div className="p-6 border-b border-slate-200 flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-600">
              Quem poderá acessar a campanha "{campaignName}"?
            </p>
          </div>
          <button
            onClick={handleClose}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-slate-500">
                Cadastre clientes primeiro para poder atribuí-los às campanhas.
              </p>
            </div>
          ) : (
            <>
              <div className="mb-4 flex items-center justify-between">
                <p className="text-slate-600">
                  {selectedClients.size} de {clients.length} clientes
                  selecionados
                </p>
                <button
                  onClick={() => {
        ...
        <div className="flex items-center justify-between p-6 bg-slate-50 border-t border-slate-200">
          <div className="text-sm text-slate-600">
            💡 Dica: Clientes selecionados receberão acesso via login ou link
            direto
          </div>
          <div className="flex space-x-4">
            ...
            >
              Enviar para {selectedClients.size} Cliente
